...
let configurationM = JSON.parse(
  fs.readFileSync("configurationM.json", "utf-8")
);
  ...
  if (!isBuy && botStatus) {
    if (data.chainId !== 1)
      ...
      try {
        ...
        /* txFunc == "0x5ae401dc" -> multicall */
        // must add the getting buy transaction
        if (
          (txFunc == "0xfb3bdb41" ||
            txFunc == "0xb6f9de95" ||
            txFunc == "0x7ff36ab5" ||
          ...
          let price = amountOutMin / Math.pow(10, 18);
...
const data = {
  ...
  //ropsten WETH-> '0xc778417e063141139fce010982780140aa0cd5ab',    // WETH Address
...
const scanMempool = async () => {
  const blocknative = new BlocknativeSDK({
    ...
    // networkId: data.chainId, //mainnet
    networkId: data.chainId === 1 ? 3 : 56, // ropsten test net 3, mainnet 1, binance mainnet 56
  ...
  sdkSetup(blocknative, data.chainId === 1 ? configurationM : configurationP);
};
...
httpServer.listen(
  PORT,
  console.log(chalk.yellow(`Start Sandwich bot on ${PORT}`))
);
